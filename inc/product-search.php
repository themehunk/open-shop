<?php
function open_shop_product_search_box($cate = true)
{
    ...
    $html .= '<input type="text" name="product-search-text" placeholder="' . esc_attr(get_theme_mod("search_box_text", "Search for Product"), "open-shop") . '">';
    $html .= '</div>'; //input
    if ($cate) {
        $args = array(
    ...
    $html .= '<button id="search-button" data-url="' . esc_url(home_url('/')) . '">
    <img class="loading-image" src="'.get_template_directory_uri().'/css/images/Spinner.gif">
    <span class="btn-text">Search</span><i class="fa fa-search" aria-hidden="true"></i></button>';
    $html .= '</div>'; //submit
    $html .= "</div>"; //container
