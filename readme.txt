== Theme: Open Shop ==
Contributors: ThemeHunk
Tags: wide-blocks, block-patterns, block-styles, editor-style, e-commerce, blog, grid-layout, one-column, two-columns, three-columns, four-columns, left-sidebar, right-sidebar, custom-background, custom-colors, custom-header, custom-logo, custom-menu, add_editor_style, featured-image-header, featured-images, footer-widgets, full-width-template, sticky-post, theme-options, threaded-comments, translation-ready
Requires at least: WordPress 5.0
Requires PHP: 7.0
Tested up to: 7.0
Stable tag: 1.7.6
License: GPLv3 or later
License URL: https://www.gnu.org/licenses/gpl-3.0.en.html


== Description ==
Open shop is a fast and responsive shopping WordPress theme with deeply integration of Woocommerce plugin. Best suited for furniture, grocery, clothing, electronic, food, home appliances site, gadget store, jewellery shop, fashion shop and decorative stores , you can create any type of ecommerce website by using it. It is also RTL ready. It contains multiple widgets options and layouts, Some highlighted features are Advance product search, Multiple Hero slider layouts, Product carousel , Product carousel with tabbed category filter, Woo Category slider, Product list layout, Top header with three layouts, Ribbon section, Call to Action section, About Us widget, Blog post widget slider, Footer bottom with three layouts. Open shop is best theme for your next website, check it demos: https://wpthemes.themehunk.com/open-shop
For more information about Theme https://www.themehunk.com

== Frequently Asked Questions ==

= Does your theme support builder =
Yes our theme supports builder

= Can i use your theme with latest version of WordPress=
Yes, this theme is tested with latest version of WordPress, you can use it for your site.

== Changelog ==

= Version 1.7.6 =
* Update: Style Improved.

= Version 1.7.5 =
* Update: Wishlist Text made to be update by plugins settings.

= Version 1.7.4 =
* Update: Style updated.
* Update: Title height issue resolved.

= Version 1.7.3 =
* Update: Cartdropdown issue resolved.
* Update: layout position issue resolved.
* Update: category list modified and flyout style added

= Version 1.7.2 =
* Update: Ajax Nonce verification added in wp_ajax_default_home, wp_ajax_th_activeplugin.
* Update: Theme search & cart unused code removed.

= Version 1.7.1 =
* Update: .woocommerce div.product form.cart.variations_form css added.
* Update: TH Store One Added.

= Version 1.7.0 =
* Update: Category height issue resolved.

= Version 1.6.9 =
* Update: Svg Icons replaced.
* Update: Style updated & cart icon issue resolved.
* Update: Header icon structure updated and style modified.

= Version 1.6.8 =
* Update: Category menu hover issue resolved.

= Version 1.6.7 =
* Update: Theme preview style improved.

= Version 1.6.6 =
* Update: get_template_part removed by require_once.
* Update: Category scroll issue resolved.

= Version: 1.6.5 =
1. Child theme Fatal error issue fixed 

= Version: 1.6.4 =
1. TH COMPARE INETEGRATED.
2. Style Improved
3. Tested with latest PHP 8.3 / 8.4.
4. Tested with wordpress 6.8.
5. No open code present (__load_text_domain error)
6. TH Wishlist added in Option Panel.
7. Hooks added at after & before of haeder and footer also class added in these tags.
8. Product layouts style issue resolved.
9. Update: Header implementation through hooks using do_action( 'open_shop_header' );
10. Update: Footer implementation through hooks using do_action( 'open_shop_footer' );
11. Update: Absolute path in th-icon issue resolved.
12. Update: Style improved related products in single page.
13. Update: List view in shop page style improved.

= 1.6.3 =
* Update: Woocommerce checkout page .woocommerce-checkout .thunk-content-wrap {
    display: -webkit-flex;
} removed now overflow:hidden added to work with both old and new checkout page.

= 1.6.2 =
* Update: * Update: Customizer widget redirect issue resolved.

= 1.6.1 =
* Update: Tested with WordPress 6.8.1 & style updated.

= 1.6.0 =
* Update: _load_textdomain_just_in_time was called incorrectly issue resolved.
* Update: Tested with WordPress 6.8.

= 1.5.9 =
* Update: Recommended plugins updated.

= 1.5.8 =
* Fix: Style improved.

= 1.5.7 =
* Fix: Homepage disable sidebar issue resolved.

= 1.5.6 =
* New: Theme limitation edition added.
* New: Header categories to include added.

= 1.5.5 =
* Fix : open_shop_is_json issue resolved.
* Update: Banner display conditional added.

= 1.5.4 =
* New: Plugin activation added in admin area and customizer.

= 1.5.3 =
* Update: New Theme.json version 3 added.
* Update: Elementor full width category and page layout issue resolved.

= 1.5.2 =
* Update: compatible with wordpress 6.6 .

= 1.5.1 =
* Update: New import feature added.
* Update : New Demo added.
* Update : X share button improve.
* Update : Dashbord button text and style change.
* Remove : Extra plugin install removed.

= 1.5.0 =
* Theme tested with WordPress 6.3

= 1.4.9 =
* Editor.css Added.

= 1.4.8 =
* Style improved & block support added.
* Th Icon added in category page.
* Blank Template added.
* Move To Top style improved.
* Tags Added.
* Menu Style improved.

= 1.4.7 =
* wp-block-latest-comments style improved.
* Theme Checked with WordPress 6.1.

= 1.4.6 =
* TH Compare Pro Integrated.

= 1.4.5 =
* Footer Issue resolved.

= 1.4.4 =
* Seo Optimization & Below Footer Copyright.

= 1.4.3 =
* Option panel added new plugin.
* Style issue fixd.

= 1.4.2
* Sitch theme hook added.
* Icon Improved
* Style issue fixed.

= 1.4.1 =
* Style issue fixed.

= 1.4.0 =
* Th option panel notification udpated.
* Add pro and docs button in customizer.
* Below footer copyright section disable.
* Th all in one woo cart plugin added.
* Th option panel sidebar text updated.
* Th all in one woo cart plugin class taiowc-fly-cart added.
* Remove theme support for widget block editor.
* Th all in one woo cart responsive issue fixed.


= 1.3.9 =
* Th product compare plugin added.
* Th advance product search pro plugin issue fixed.

= 1.3.8 =
* Th option panel updated.
* Child theme notification added.

= 1.3.7 =
* Th advance product search plugin url issue fixed.

= 1.3.6 =
* Th option panel updated.
* child theme tab removed.
* option panel style issue fixed.

= 1.3.5 =
* upgrade button link fix.

= 1.3.4 =
* Option panel design updated.

= 1.3.3 =
* Th advance product search plugin installation message style improved.

= 1.3.2 =
* Th advance product search plugin added.
* Th advance product search msg issue fixed.
* Checkout page search button issue fixed.

= 1.3.1 =
* Sticky product search issue fixed.

= 1.3.0 =
* Yith Plugin added.
* Product search updated.
* Theme make compatible with Th variation swatches plugin

= 1.2.9 =
* Customizer Draft issue fixed.
* Post Publish Issue fixed.
* Image cropping issue fixed.
* Sidebar widget font issue fixed.
* Footer widtet font issue fixed.

= 1.2.8 =
* Product search updated.
* Th variation swatches plugin added.

= 1.2.7 =
* Theme Support link changed.

= 1.2.6 =
* Style Issue Fixed.
* body font issue fixed.
* rating color issue fixed.
* pricing font size improve.

= 1.2.5 =
* Style Issue Fixed.

= 1.2.4 =
* Product loop Improved.
* Front Product limit option Add Product style Section in Customizer.
* Slider Speed added.
* Style issue resolved.
* Header Image issue resolved.

= 1.2.3 =
* Rtl Issue fixed.
* Enable mobile cart count in header.
* Increase category sub menu arrow width, height and add color.

= 1.2.2 =
* WPC Icons added.
* Useful Plugin tab added.
* Plus Minus buttons changed on single product page.
* Issues resolved.
* Translation issue language folder resolved.

= 1.2.1 =
* Rtl Issue Fixed.

= 1.2.0 =
* Woocomerce single page style improved.

= 1.1.9 =
* Star rating style improved.

= 1.1.8 =
* Language file update.

= 1.1.7 =
* Rtl issue fixed.

= 1.1.6 =
* ThemeHunk Megamenu add in Recommended Plugins.

= 1.1.5=
* Improved Widget js function.

= 1.1.4=
* Category function improved.
* Shop page product link issue fixed.

= 1.1.3=
* Header call to action issue fixed.

= 1.1.2=
* Full Rtl compatible.

= 1.1.1=
* sidebar notice issue fixed.
* add link over top slider.
* translation string issue fixed.

= 1.1.0=
* Add to cart Issue Fixed.
* Category order fixed on home page.
* Group product issue fixed.
* Style issue fixed.

= 1.0.9=
* Option panel issue fixed.

= 1.0.8=
* All issue fixed.

= 1.0.7=
* All issue fixed.

= 1.0.6 =
* Style Issue fixed.

= 1.0.5 =
* wp_body_open function added.

= 1.0.4 =
* Text domain issue Fixed.

= 1.0.3 =
* Responsive sidebar issue Fixed.

= 1.0.2 =
* Menu Issue Fixed.

= 1.0.1 =
* Issue Fixed.

= 1.0.0 =
* Released: February, 20, 2020

Initial release

== Upgrade Notice ==

= Version 1.6.9 =
* Update: Header icon style issue resolved.

== Resources ==
 
License for images:

1. Image used in Screenshots.
Image Name: screenshot.png
Resource link: https://stocksnap.io/photo/K8VEP08K07
Licensed under the CCO license.
License link : https://stocksnap.io/license

https://wordpress.org/plugins/woocommerce/
All are WooCommerce Sample Data Image
License: GPLv3

== Theme License & Copyright ==
Open Shop is distributed under the terms of the GNU GPL
Open-Shop-Copyright 2020, themehunk.com
License : GPLv3 or later
License URL: https://www.gnu.org/licenses/gpl-3.0.en.html

== animate.css ==
animate.css, Copyright Daniel Eden License: MIT  Source : https://github.com/daneden/animate.css

== OpenSans Font ==
License: SIL OFL 1.1
License URL: http://scripts.sil.org/OFL

== OpenSans Font ==
License: SIL OFL 1.1
License URL: http://scripts.sil.org/OFL

== Roboto License ==
License: SIL OFL 1.1
License URl for Font : http://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL

== Font Awesome CSS license ==
License: MIT License
License URL for CSS : http://fontawesome.io/license (Font: SIL OFL 1.1)

== normalize.css ==
License: MIT
Source: https://necolas.github.io/normalize.css/

== breadcrumb.php ==
breadcrumbs.php, Copyright Justin Tadlock License: GNU GPL License, version 0.2.1 Source: https://github.com/justintadlock/breadcrumb-trail/

==  Other Licenses ==
Other js files are self coded.

Once again, thank you so much for trying the Open Shop WordPress Theme. We will be glad to help you. If you have any questions related to this theme then do let us know & we will try our best to assist. If you have any general questions related to the themes on ThemeHunk, we would recommend you to visit the ThemeHunk Support Forum and ask your queries there.